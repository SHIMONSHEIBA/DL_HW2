...
do -- data augmentation module
  ...
  function BatchFlip:updateOutput(input)
    if self.train then
      ...
      local flip_mask = torch.randperm(bs)--:le(bs/2)
      for i=1,input:size(1) do
       	if (flip_mask[i] % 4 == 0) then image.hflip(input[i]) end
	if (flip_mask[i] % 4 == 1) then image.vflip(input[i]) end
	if (flip_mask[i] % 4 == 2) then image.RandomCrop(input[i],tl,32,32) end
