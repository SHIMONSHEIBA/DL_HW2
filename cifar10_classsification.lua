...
model:add(cudnn.LeakyReLU(true))                          -- ReLU activation function
...
model:add(cudnn.LeakyReLU(true))
--model:add(cudnn.SpatialConvolution(64, 32, 3, 3))
model:add(cudnn.SpatialConvolution(16, 32, 5, 5, 2, 2, 2, 2))
model:add(cudnn.View(32*4*4):setNumInputDims(3))  -- reshapes from a 3D tensor of 32x4x4 into 1D tensor of 32*4*4
model:add(cudnn.Linear(32*4*4, 32))             -- fully connected layer (matrix multiplication between input and weights)
--model:add(cudnn.ReLU(true))
model:add(cudnn.LeakyReLU(true))
model:add(cudnn.Dropout(0.2))                      --Dropout layer with p=0.5
model:add(cudnn.Linear(32, #classes))            -- 10 is the number of outputs of the network (in this case, 10 digits)
model:add(cudnn.LogSoftMax())                     -- converts the output to a log-probability. Useful for classificati
