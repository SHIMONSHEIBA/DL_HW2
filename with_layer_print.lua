...
local output2 = conv2:forward(img)
print('second layer output: ' .. output2:size())

local conv3 = nn.ReLU(true)
conv3:cuda()
print('third layer: ' .. conv3)
local output3 = conv3:forward(img)
print('third layer output: ' .. output3:size())

local conv4 = nn.SpatialBatchNormalization(32)
conv4:cuda()
print('4th layer: ' .. conv4)
local output4 = conv4:forward(img)
print('4th layer output: ' .. output4:size())

local conv5 = nn.SpatialConvolution(32, 64, 3, 3)
conv5:cuda()
print('5th layer: ' .. conv5)
local output5 = conv5:forward(img)
print('5th layer output: ' .. output5:size())

local conv6 = nn.SpatialMaxPooling(2,2,2,2)
conv6:cuda()
print('6th layer: ' .. conv6)
local output6 = conv6:forward(img)
print('6th layer output: ' .. output6:size())

local conv7 = nn.ReLU(true)
conv7:cuda()
print('7th layer: ' .. conv7)
local output7 = conv7:forward(img)
print('7th layer output: ' .. output7:size())

local conv8 = nn.SpatialBatchNormalization(64)
conv8:cuda()
print('8th layer: ' .. conv8)
local output8 = conv8:forward(img)
print('8th layer output: ' .. output8:size())

local conv9 = nn.SpatialConvolution(64, 32, 3, 3)
conv9:cuda()
print('9th layer: ' .. conv9)
local output9 = conv9:forward(img)
print('9th layer output: ' .. output9:size())
