...
saveTensorAsGrid(trainData:narrow(1,100,36),'train_100-136.jpg') -- display the 100-136 images in dataset, train_100-136.jpg- the name of the file
...
local output = conv:forward(img) --forward on 1 picture
print(output:size()) --print the output size
saveTensorAsGrid(output, 'convOut.jpg') --save the output - the feature map

local weights = conv.weight
saveTensorAsGrid(weights, 'convWeights.jpg') --save the weights,  the filter itself
print(weights:size())
...
for i=1,3 do -- over each image channel
    mean[i] = trainData[{ {}, {i}, {}, {}  }]:mean() -- mean estimation for each color seperatly
...
local Weights_1st_Layer = model:get(1).weight --all the we from the first layer, look on all the filters of the layer
...
for l=1,9 do
  print('Layer ' ,l, tostring(model:get(l))) --get to the l node - the l layer
